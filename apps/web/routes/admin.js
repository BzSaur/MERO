...
router.post('/usuarios', async (req, res) => {
  try {
    ...
    const raw = err.response?.data?.message || 'Error al crear usuario';
...
router.post('/usuarios/:id', async (req, res) => {
  try {
    ...
    const raw = err.response?.data?.message || 'Error al actualizar';
    req.flash('error', Array.isArray(raw) ? raw.join(', ') : raw);
...
router.post('/catalogos/areas/:id/eliminar', async (req, res) => {
  try {
    ...
    const raw = err.response?.data?.message || 'Error al eliminar';
    req.flash('error', Array.isArray(raw) ? raw.join(', ') : raw);
    res.redirect('/admin/catalogos/areas');
  }
});
