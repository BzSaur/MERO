...
async function resetSequences() {
  ...
  for (const tabla of tablas) {
    try {
      ...
      `);
      console.log(`   secuencia sincronizada: ${tabla}`);
    } catch (error) {
      console.warn(`   aviso: no se pudo sincronizar secuencia de ${tabla}`, error);
    }
  }
}
...
async function main() {
  ...
  // Empleados se sincronizan desde VITA
  ...
  for (const u of USUARIOS) {
    const existente = await prisma.meroUsuario.findUnique({
      where: { email: u.email },
    });

    const passwordHash = u.email === 'admin@mero.local' ? adminHash : userHash;

    if (existente) {
      await prisma.meroUsuario.update({
        where: { email: u.email },
        data: {
          nombre: u.nombre,
          passwordHash,
  ...
  console.log(`   ${actualizados} usuarios actualizados`);

  // Fix importante: alinear secuencias después de insertar IDs manuales
